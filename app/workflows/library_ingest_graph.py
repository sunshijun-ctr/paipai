...
from app.workflows.base import AgentWorkflowState, BuildAgentInput, ProgressCallback

logger = logging.getLogger(__name__)
_SUPPORTED_INGEST_EXTS = {".pdf", ".pptx", ".txt", ".md", ".text", ".rst"}


def build_library_ingest_graph(
    *,
    workflow_name: str,
    ...
    builder.add_node("detect_file_source", _detect_file_source_node)
    builder.add_node("validate_files", _validate_files_node)
    builder.add_node("library_ingest", _library_ingest_node(progress))
    builder.set_entry_point("detect_file_source")
    builder.add_edge("detect_file_source", "validate_files")
    builder.add_conditional_edges(
        "validate_files",
        _ingest_route,
        {
            "ingest": "library_ingest",
            "end": END,
        },
    )
    builder.add_edge("library_ingest", END)
    return builder.compile()


async def _detect_file_source_node(state: AgentWorkflowState) -> AgentWorkflowState:
    ...
    selected.extend(_extract_local_file_targets(state["user_query"], intent))
    ...
    state["agent_names"] = ["library_tool"]
    state["total_agents"] = 1
    state["done_agents"] = 0
    return state


async def _validate_files_node(state: AgentWorkflowState) -> AgentWorkflowState:
    task_state = state["task_state"]
    candidates = task_state.working_memory.get("library_ingest_candidates", [])
...
def _library_ingest_node(progress: ProgressCallback):
    async def node(state: AgentWorkflowState) -> AgentWorkflowState:
        task_state = state["task_state"]
        valid = task_state.working_memory.get("library_ingest_valid", [])
        invalid = task_state.working_memory.get("library_ingest_invalid", [])
        ...
        for paper in valid:
            title = str(paper.get("title") or os.path.basename(str(paper.get("local_path") or "")) or "Untitled")
        ...
        state["done_agents"] = int(state.get("done_agents", 0)) + 1
...
def _ingest_route(state: AgentWorkflowState) -> str:
